export class ModelColor {
    ...
    private static readonly COLOR_TABLE = ModelColor.initColorTable();
    private static readonly HSB_TO_RGB = ModelColor.initHsbToRgb(0.7, 0, 512);

    static initColorTable(): Uint32Array {
    ...
    static initHsbToRgb(arg0: number, arg1: number, arg2: number): Uint32Array {
        const table = new Uint32Array(65536);
        arg0 += Math.random() * 0.03 - 0.015;
        let tableIndex = arg1 * 128;
        for (let i = arg1; i < arg2; i++) {
            const d = (i >> 3) / 64.0 + 0.0078125;
            const d_59_ = (i & 0x7) / 8.0 + 0.0625;
